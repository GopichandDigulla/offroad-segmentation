"""
SegFormer-2 Test/Inference Script
...
def save_prediction_comparison(img_tensor, gt_mask, pred_mask, output_path, data_id, iou_score):
    """Save a side-by-side comparison with IoU text"""
    ...
    # 🔥 IoU added here
    plt.suptitle(f'Sample: {data_id} | IoU: {iou_score:.4f}', fontsize=14)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SegFormer-B2 prediction/inference script')
    ...
    valset = MaskDataset(data_dir=args.data_dir, img_size=img_size)
    ...
    with torch.no_grad():
        ...
        for batch_idx, (imgs, labels, data_ids) in enumerate(pbar):
            ...
            predicted_masks = torch.argmax(upsampled, dim=1)

            # Calculate metrics
            iou, class_iou = compute_iou(upsampled, labels, num_classes=n_classes)
            dice, class_dice = compute_dice(upsampled, labels, num_classes=n_classes)
